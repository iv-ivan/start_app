...
class FrontendCdkStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        oac = aws_cloudfront.S3OriginAccessControl(self, "OAC",)

        behavior_without_cache = aws_cloudfront.BehaviorOptions(
                   origin=aws_cloudfront_origins.S3BucketOrigin(website_bucket, origin_access_control_id=oac.origin_access_control_id),
        ...
        distribution = aws_cloudfront.Distribution(self,
               "WebsiteDistribution",
        ...
        aws_route53.ARecord(self, "AliasRecord",
            zone=hosted_zone,
            record_name=DOMAIN,
            target=aws_route53.RecordTarget.from_alias(aws_route53_targets.CloudFrontTarget(distribution)),
            )

        # Optionally, add a record for www subdomain (if using one)
        aws_route53.ARecord(self, "AliasRecordWWW",
            zone=hosted_zone,
