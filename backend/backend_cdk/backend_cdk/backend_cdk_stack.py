    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        container = task_definition.add_container(
            ...
            image=aws_ecs.ContainerImage.from_ecr_repository(
                ecr_repo, tag="latest"
            ),  # take "latest" Docker image
            logging=aws_ecs.LogDrivers.aws_logs(
                stream_prefix="ecs"
            ),  # easiest setup to view service logs in AWS
        ...
        listener_http = alb.add_listener("HttpListener", port=80, open=True)
            ...
            listener.add_targets(
                ...
                targets=[
                    service.load_balancer_target(
                        container_name="Container", container_port=80
                    )
                ],
